  ...
  <div>
    <el-form>
      <el-form-item label="添加直属用户">
          <el-select v-model="selectedName" placeholder="请选择需要添加的用户" class="user_el-select" @change='boxSelected' filterable>
            <el-option v-for="item in directUserListSelect" :key="item.uuid" :label="item.fullName" :value="item.uuid" :disabled="item.isDisabled"> </el-option>
          </el-select>
      </el-form-item>
    </el-form>
        ...
        getDepartmentUserVoList(listQueryParm)
          .then(
            function (result) {
              ...
              getRoleUser(listQueryParm.userType)
                .then(
                  function (result) {
