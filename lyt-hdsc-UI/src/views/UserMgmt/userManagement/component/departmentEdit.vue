<template>
  <div class='usermgn'>
    ...
    <el-form :model='department' :rules="rules" ref="department" v-show="showMainDepartment">
      <div>
        <el-form-item label='用户类型' prop='userType' :label-width="formLabelWidth">
          <el-select v-model='department.userType' class="user_el-select" placeholder="请选择用户类型" disabled>
    ...
    <div v-show="showDirectDepartment">
      <direct-department ref="directDepartmentVue" :departmentUuidValue="curDepartmentUuidParm" @gridDirecDepCreateEvent="refreshDir" :userTypeValue="userTypeParm"></direct-department>
    </div>
    <div v-show="showUserDepartment">
      <direct-user ref="directUserVue" :departmentUuidValue="curDepartmentUuidParm" :userTypeValue="userTypeParm"></direct-user>
    </div>
  </div>
</template>
  ...
  methods: {
    handleSubTabClick (tab, event) {
      if (tab.name === '0') {
        ...
        this.listQuery.userType = this.department.userType
        this.userTypeParm = this.department.userType
