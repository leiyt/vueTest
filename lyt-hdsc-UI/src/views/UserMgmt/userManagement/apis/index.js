...
export const listUserType = () => {
  return Axios.get(contextPath + '/usermgmt/maindata/getDictData?dictType=COURT_USER_TYPE'
  ).then(res => res.data)
}
...
export const getUserGroupList = (query) => {
  return Axios.get(contextPath + '/usermgmt/usergroup/list?userGroupName=' + encodeURI(query.userGroupName) + '&currentPage=' + query.currentPage + '&pageSize=' + query.pageSize + '&userType=' + query.userType
  ).then(res => res.data)
}
...
export const getDepartmentList = (listQuery) => {
  ...
  '&departmentName=' + encodeURI(listQuery.q_departName)
  ).then(res => res.data)
}
// 删除部门信息
...
export const getDirectDepartmentSelect = (uuid, departmentType) => {
  console.log('getDirectDepartmentSelect<<<<<uuid:' + uuid)
  return Axios.get(contextPath + '/usermgmt/department/listExAllparSelfDirDep?departmentUuid=' + uuid + '&userType=' + departmentType
  ).then(res => res.data)
}
// 当前部门上级及本体以外的部门节点(追加上级部门下拉框)
export const getParenetDepartmentSelect = (uuid, departmentType) => {
  console.log('getParenetDepartmentSelect<<<<<uuid:' + uuid)
  return Axios.get(contextPath + '/usermgmt/department/listExDirparSelfChildrenDep?departmentUuid=' + uuid + '&userType=' + departmentType
  ).then(res => res.data)
}
// 查询当前部门下的直属部门列表
export const getChildrenDepartmentVoList = (listQuery) => {
  ...
  console.log('<<<<<listQuery.page:' + listQuery.page)
  console.log('<<<<<listQuery.limit:' + listQuery.limit)
  console.log('<<<<<listQuery.userType:' + listQuery.userType)
  ...
  '&departmentUuid=' + listQuery.departmentUuid + '&userType=' + listQuery.userType
  ).then(res => res.data)
}
// 删除下属部门
...
export const getRoleList = (query) => {
  return Axios.get(contextPath + '/usermgmt/role/list?currentPage=' + query.currentPage + '&pageSize=' + query.pageSize + '&userType=' + query.userType
  ).then(res => res.data)
}
// // 查询角色完整清单
...
// }
// 从Maindata查询角色完整清单
export const getRoleListAllMaindata = (userType) => {
  return Axios.get(contextPath + '/usermgmt/maindata/listRole?userType=' + userType
  ).then(res => res.data)
}
...
// 获取用户组清单
export const getRoleUserGroup = (userType) => {
  return Axios.get(contextPath + '/usermgmt/maindata/listUsergroup?userType=' + userType
  ).then(res => res.data)
}
// 获取用户组筛选清单
...
// 获取用户清单
export const getRoleUser = (userType) => {
  return Axios.get(contextPath + '/usermgmt/maindata/listUser?userType=' + userType
  ).then(res => res.data)
}
// 获取资源清单
...
export const getUserListByPage = (listQuery) => {
  ...
  '&userName=' + encodeURI(listQuery.q_userName) + '&fullName=' + encodeURI(listQuery.q_fullName) + '&primaryPhone=' + listQuery.q_primaryPhone + '&userType=' + listQuery.userType
  ).then(res => res.data)
}
// 校验用户名是否唯一
...
// 获取部门下拉框
export const getDepartmentOptions = (userType) => {
  return Axios.get(contextPath + '/usermgmt/maindata/listDepartment?userType=' + userType
  ).then(res => res.data)
}
// 获取用户状态下拉框
...
// --------------用户组--------------
// 获取用户组下拉框列表信息
export const getUserGroupListSelect = (userType) => {
  return Axios.get(contextPath + '/usermgmt/maindata/listUsergroup?userType=' + userType
  ).then(res => res.data)
}
// 将当前用户添加到某用户组中
...
// --------------关联角色--------------
// 获取当前用户关联角色下拉框列表信息
export const getUserRoleListSelect = (userType) => {
  return Axios.get(contextPath + '/usermgmt/maindata/listRole?userType=' + userType
  ).then(res => res.data)
}
// 将当前用户添加到某角色中
...
export const checkResourceCode = (listParm) => {
  ...
  console.log('<<<<<uuid:' + listParm.uuid)
  return Axios.get(contextPath + '/usermgmt/resource/isExist?resourceType=' + listParm.resourceType +
  '&uuid=' + listParm.uuid + '&appCode=' + encodeURI(listParm.appCode)
  ).then(res => res.data)
}
// 获取主设备类型
