        ...
        <el-col :span="4">
          <div>
            <el-form :inline="true" :model="formInline" class="demo-form-inline">
              <el-form-item label="">
                <el-button v-if="$route.params.versionId && displayVersionStatus != 'mm.versts.disable' && versionPublishStatus == 'mm.pubsts.publish'" :disabled="loadingVersion" @click="showAddModelDialog()" type="primary"
        ...
        <el-col :span="20">
          <div class="text-right">
            ...
            <el-form :inline="true" :model="modelListSearch" :rules="searchRules" ref="modelListSearch" class="demo-form-inline">
                ...
                <el-form-item label="计划来源" prop="taskSource">
  ...
  export default {
    ...
    methods: {
      showMessageFromChild (data) {
        ...
        // console.info(data)
        this.displayVersionStatus = data.versionStatus
        this.versionPublishStatus = 'mm.pubsts.none'
        if (data && data.statusInMNodes && data.statusInMNodes.length > 0) {
          this.versionPublishStatus = data.statusInMNodes[0].publishStatus
        }
      },
      ...
      handleClose (done) {
        if (this.$refs['newTaskPlan']) {
          this.$refs['newTaskPlan'].clearValidate()
        }
        if (this.$refs['editTaskPlan']) {
          this.$refs['editTaskPlan'].clearValidate()
        }
        this.addModelDialogVisible = false
        this.editModelDialogVisible = false
        this.uploadDialogVisible = false
